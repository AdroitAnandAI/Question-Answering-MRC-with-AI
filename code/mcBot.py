...
from official_eval_helper_noid import get_json_data, get_answer, generate_answers_prob
...
def main(unused_argv):
    ...
    # print qn_uuid_data[0], context_token_data[0], qn_token_data[0]
    ...
    with tf.Session(config=config) as sess:
        ...
        print "\n\n*** MACHINE COMPREHENSION BOT ***\n"
        question = raw_input("\n<MRC Bot > ")
        while (question != 'q'):

            # if input length is more then it is a passage.
            if (len(question) > 100):
                context_token_data = question.split()
                context_token_data = map(unicode,context_token_data)
                # print (context_token_data)
                print "<MRC Bot >  Analyzed passage. What would you like to know?\n"
            else:
                qn_token_data = question.split()
                qn_token_data = map(unicode,qn_token_data)
                # print (qn_token_data)
                answer = get_answer(sess, qa_model, word2id, [context_token_data], [qn_token_data])
                print "<MRC Bot > " + answer.encode('ascii','ignore')
                
            question = raw_input("<MRC Bot > ")
