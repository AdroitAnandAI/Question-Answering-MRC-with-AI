...
from data_batcher_noid import padded, Batch
...
def refill_batches(batches, word2id, context_token_data, qn_token_data, batch_size, context_len, question_len):
    """
    ...
    # Get next example
    context_tokens, qn_tokens = readnext(context_token_data), readnext(qn_token_data)

    while context_tokens and qn_tokens:
        ...
        examples.append((context_tokens, context_ids, qn_ids))
    ...
    for batch_start in xrange(0, len(examples), batch_size):
        context_tokens_batch, context_ids_batch, qn_ids_batch = zip(*examples[batch_start:batch_start + batch_size])

        batches.append((context_tokens_batch, context_ids_batch, qn_ids_batch))

    return



def get_batch_generator(word2id, context_token_data, qn_token_data, batch_size, context_len, question_len):
    """
    ...
    while True:
        if len(batches) == 0:
            refill_batches(batches, word2id, context_token_data, qn_token_data, batch_size, context_len, question_len)
        if len(batches) == 0:
            break

        # Get next batch. These are all lists length batch_size
        (context_tokens, context_ids, qn_ids) = batches.pop(0)
...
def get_answer(session, model, word2id, context_token_data, qn_token_data):
    """
    Given a model, and a set of (context, question) pairs, each with a unique ID,
    use the model to generate an answer for each pair, and return a dictionary mapping
    each unique ID to the generated answer.
    ...
    for batch in get_batch_generator(word2id, context_token_data, 
        qn_token_data, model.FLAGS.batch_size, model.FLAGS.context_len, model.FLAGS.question_len):
        ...
        # print "Prediction start end are:"
        # print pred_start_batch, pred_end_batch
        # print "Context paragraphs are:"
        # print batch.context_tokens[0]
        # print "----------------------"
        ...
        for ex_idx, (pred_start, pred_end) in enumerate(zip(pred_start_batch, pred_end_batch)):
            ...
            # Detokenize and add to dict
            answer = detokenizer.detokenize(pred_ans_tokens, return_str=True)

        batch_num += 1

    # print "Finished getting answers for qn"
